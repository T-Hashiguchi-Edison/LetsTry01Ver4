...
namespace パソコンデータ入力
{
    public partial class Frm顧客伝票修正 : Form
    {
        ...
        public Frm顧客伝票修正(FormCommon.Form_IF prm)
        {
            InitializeComponent();
        ...
        //              Form　Load
        /// </summary>
        private void Frm顧客伝票修正_Load(object sender, EventArgs e)
        {
            //初期表示
        ...
        private void SetPnlInputOrigin()
        {
            ...
            PnlInput.Size = new Size(544, 616);
            {
                ...
                TxtCustCd.Location = new Point(152, 216);
                TxtCustCd.Size = new Size(352, 28);
                TxtCustCd.Font = new Font("ＭＳ Ｐゴシック", 15.75f);
                ...
                TxtItemCd.Location = new Point(152, 272);
                TxtItemCd.Size = new Size(352, 28);
                TxtItemCd.Font = new Font("ＭＳ Ｐゴシック", 15.75f);
                ...
                TxtTelNo.Location = new Point(152, 328);
                TxtTelNo.Size = new Size(352, 28);
                ...
                TxtMailAddress.Location = new Point(152, 384);
                TxtMailAddress.Size = new Size(352, 28);
        ...
        private void DispErrCard()
        {
            ...
            //顧客コード
            TxtCustCd.Text = CurInputData.CustCd;

            //商品コード
            TxtItemCd.Text = CurInputData.ItemCd;

            //電話番号
            TxtTelNo.Text = CurInputData.TelNo;

            //メールアドレス
            TxtMailAddress.Text = CurInputData.MailAddress;

            //フォーカスセット
            TxtCustCd.Focus();
            TxtCustCd.Select(0, 0);
                        ...
                        using (SQLiteCommand command = new SQLiteCommand(sql, con))
                        {
                            using (SQLiteDataReader reader = command.ExecuteReader())
                            {
                                while (reader.Read())
                                {
                                    ...
                                    CurInputData.CustCd = reader.GetString(1);
                                    CurInputData.ItemCd = reader.GetString(2);
                                    CurInputData.TelNo = reader.GetString(3);
                                    CurInputData.MailAddress = reader.GetString(4);
                                    CurBeforeData.CustCd = CurInputData.CustCd;
                                    CurBeforeData.ItemCd = CurInputData.ItemCd;
                                    CurBeforeData.TelNo = CurInputData.TelNo;
                                    CurBeforeData.MailAddress = CurInputData.MailAddress;
                                }
                            }
                        }
                        if (CurInputData.id == "")
        ...
        private Boolean CheckInput(Boolean DispFlg)
        {
            ...
            //顧客コード
            if (TxtCustCd.Text != LblCustCd_C.Text)
            {
                CurCorrectData.CustCd = "×";

                //文字チェック
                CheckChar(LblCustCd_C.Text, TxtCustCd.Text);
            ...
            CurInputData.CustCd = TxtCustCd.Text;

            //商品コード
            if (TxtItemCd.Text != LblItemCd_C.Text)
            {
                CurCorrectData.ItemCd = "×";

                //文字チェック
                CheckChar(LblItemCd_C.Text, TxtItemCd.Text);
            ...
            CurInputData.ItemCd = TxtItemCd.Text;
        ...
        private void UpdateCardNo()
        {
            try
            {
                ...
                using (SQLiteConnection con = new SQLiteConnection(SetconnectionString(connectionStringD)))
                {
                    ...
                    //ユーザマスタを取得して、表示する
                    sql = "UPDATE ユーザマスタ SET Start_No2 = " + cardNO.ToString() + ","
        ...
        private void OutputResultText()
        {
            ...
            try
            {
                ...
                FileInput = "解析結果\\" + ifc.Id + "\\" + StartTime.ToString("yyyyMMdd") + "_" + StartTime.ToString("HHmmss")
                          + "_入力値_顧客伝票_" + ifc.NameSi + "_" + ifc.NameMei + ".csv";

                //解析結果ファイル名
                FileResult = "解析結果\\" + ifc.Id + "\\" + StartTime.ToString("yyyyMMdd") + "_" + StartTime.ToString("HHmmss")
                           + "_解析結果_顧客伝票_" + ifc.NameSi + "_" + ifc.NameMei + ".csv";
